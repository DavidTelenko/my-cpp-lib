#pragma once
#ifndef MY_NUM_PARSER_HPP
#define MY_NUM_PARSER_HPP
...
static const char *parse_float_strtod(const char *ptr, double *outDouble) {
    ...
    *outDouble = strtod_l(ptr, &endptr, c_locale);
    ...
    // There is no realistic application where you might need values so large than
...
parse_number(const char *b, Integer &res) {
    // Negative numbers
    // Ignore - sign
    if (*b == '-') {
        if (not std::is_signed_v<Integer>) return b;
        ++b;
 ...
 *
 * @note it is relatively easy to check for data in err pointer to equal '\0',
 * if so no error ocurred. (*err != '\0'; // error occured)
 *
 * @tparam Number of any arithmetic type
 * @param str string_view of number
 * @param err const char* to error value
 * @return Parsed Number value.
 */
template <my::arithmetic Number>
inline constexpr Number parse(std::string_view str, const char *err) {
    using namespace detail;

    if constexpr (std::integral<Number>) {
        if constexpr (std::same_as<Number, bool>) {
            if (str == "true" or str == "1") return true;
            if (str == "false" or str == "0") return false;
            err = str.data();
            return false;
        } else {
            Number result{0};
            err = fast_int_parser::parse_number(str.data(), result);
            return result;
        }
    } else {
        // floats
        double result{0.0};
        err = fast_double_parser::parse_number(str.data(), &result);
...
inline constexpr std::optional<Number> parse(std::string_view str) {
    ...
    if constexpr (std::integral<Number>) {
        if constexpr (std::same_as<Number, bool>) {
            ...
            Number result{0};
            auto err = fast_int_parser::parse_number(str.data(), result);
            if (*err != '\0') return {};
            return result;
        }
    } else {
        // floats
        double result{0.0};
        auto err = fast_double_parser::parse_number(str.data(), &result);
