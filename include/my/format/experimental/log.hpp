...
#ifdef MY_LOG_PRETTY_PRINT
#include <my/format/color.hpp>
#define IF_MY_LOG_PRETTY_PRINT(expr) (expr)
#else
#define IF_MY_LOG_PRETTY_PRINT(...)
#endif
...
auto log(std::basic_ostream<Ch, Tr>& os, const Ch* format, Args&&... args) {
    my::printf(os, format, std::forward<Args>(args)...);
    os << os.widen('\n');
}
...
auto error(const Ch* format, Args&&... args) {
    IF_MY_LOG_PRETTY_PRINT(std::cerr << my::fg(LogPrintColors::ErrorColor));
    my::printf(std::cerr, "[Error]: ");
    my::log(std::cerr, format, std::forward<Args>(args)...);
    IF_MY_LOG_PRETTY_PRINT(std::cerr << my::resetcol);
}

template <class Ch,
          my::representable<std::basic_ostream<Ch>>... Args>
auto warn(const Ch* format, Args&&... args) {
    IF_MY_LOG_PRETTY_PRINT(std::cerr << my::fg(LogPrintColors::WarnColor));
...
auto info(const Ch* format, Args&&... args) {
    IF_MY_LOG_PRETTY_PRINT(std::cerr << my::fg(LogPrintColors::InfoColor));
